#!/usr/bin/env python3
"""
将中文或英文的 **Markdown 源稿**（推荐）或已有 **站内结构 HTML** 译为目标语言，并写出完整网页。

为何在 ``tools/i18n/`` 下
    **i18n** = internationalization：多语言内容、译稿、站内语言标记等，**不是**主题/版式引擎。
    本站版式由 HTML/CSS 模板决定；进站目录与索引由 ``tools/deploy`` 维护。本脚本与同目录的
    ``multi_lang_translator.py`` 等一样，负责「内容 → 某语言页面」，因此归在 i18n。

设计上的发布链路：本地 Markdown 写稿 → 本工具生成目标语 HTML → ``tools/deploy/deploy.py`` 进站
（可用 ``--deploy`` 自动调用）。

翻译与改写
    默认是忠实翻译。``--rewrite`` 打开 LLM **改写模式**（需 ``config.yaml`` 中 ``rewrite_mode`` 或使用
    DeepSeek 等支持改写的后端）。``--visitor-voice`` 在改写时附加 **面向外国游客** 的文风约束：具体、
...
用法（在仓库根目录）:
    # 在 article_to_target_page.yaml 里写好 input/target/rewrite 后，可只执行：
    python3 tools/i18n/article_to_target_page.py
    python3 tools/i18n/article_to_target_page.py --article-config tools/i18n/my-job.yaml
    # 命令行覆盖 YAML：
    python3 tools/i18n/article_to_target_page.py -i drafts/post.md --target ja
    python3 tools/i18n/article_to_target_page.py -i drafts/post.md -t en --rewrite --visitor-voice -b deepseek
    python3 tools/i18n/article_to_target_page.py -i blog/sichuan_hotpot.html --target ja

环境变量:
    DEEPSEEK_API_KEY / OPENAI_API_KEY 等（与 translate.py 一致）

文章任务配置（推荐）:
    默认读取 ``tools/i18n/article_to_target_page.yaml``（可用 ``--article-config`` 指定）。
    用于集中写 input、target、rewrite、deploy 等，命令行参数仍可覆盖 YAML。
"""
...
_URL_TRANSLATE = Path(__file__).resolve().parent.parent / "url-translate"
_I18N_DIR = Path(__file__).resolve().parent
_REPO_ROOT = Path(__file__).resolve().parent.parent.parent
_DEPLOY_SCRIPT = _REPO_ROOT / "tools" / "deploy" / "deploy.py"
_DEFAULT_ARTICLE_JOB_YAML = _I18N_DIR / "article_to_target_page.yaml"
...
def preparse_article_config_argv(argv: List[str]) -> Tuple[Path, List[str]]:
    ...
    ap.add_argument("--article-config", default=str(_DEFAULT_ARTICLE_JOB_YAML))
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="将中文或英文 Markdown 或站内 HTML 译为目标语言完整网页（复用 url-translate 翻译后端）；可对接 deploy 进站",
        epilog=(
            f"文章级参数可写在 {_DEFAULT_ARTICLE_JOB_YAML.name}（默认路径），"
    ...
    parser.add_argument(
        ...
        "--input",
        default=None,
        help="输入文件：.md（推荐）或站内结构 .html（可与 YAML 的 input 二选一，命令行优先）",
    )
